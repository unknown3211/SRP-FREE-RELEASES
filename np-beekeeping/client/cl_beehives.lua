        ...
        if inZone > 0 then
            ...
            placeObject("np_beehive", function(pPlaced, pCoords, pHeading)
                if pPlaced then
                    ...
                    local finished = exports["np-taskbar"]:taskBar(1500, "Installing beehive", false, true, false, false, nil, 5.0, PlayerPedId())
...
RegisterUICallback('np-beekeeping:addQueen', function (data)
    TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_GARDENER_PLANT", 0, true)
    local finished = exports["np-taskbar"]:taskBar(15000, "Adding Queen bee", false, true, false, false, nil, 5.0, PlayerPedId())
...
RegisterUICallback('np-beekeeping:removeHive', function (data)
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["np-taskbar"]:taskBar(15000, "Removing", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
...
AddEventHandler("np-beekeeping:harvestHive", function(pContext, pEntity)
    ...
    TriggerEvent("animation:PlayAnimation","layspike")
    local finished = exports["np-taskbar"]:taskBar(5000, "Harvesting", false, true, false, false, nil, 5.0, PlayerPedId())
    ClearPedTasks(PlayerPedId())
    if finished == 100 then
...
local function tobool(input) -- shit was being annoying so solution :)
...
function showHiveMenu(pHiveId)
    ...
    context[#context+1] = {
        ...
        description = (tobool(hive.has_queen) and "Has Queen" or "No Queen"),
    }

    --Only allow changing gender in the first 2~ stages
    if getStageFromPercent(harvest) < 2 and not tobool(hive.has_queen) then
        context[#context+1] = {
            title = 'Add Queen bee',
            key = { id = pHiveId },
